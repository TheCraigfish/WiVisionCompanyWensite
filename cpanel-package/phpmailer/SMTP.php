...
class SMTP
{
    ...
    protected $smtp_transaction_id_patterns = [
        ...
        'Microsoft_ESMTP' => '/[0-9]{3} 2.6.0 <(.*)> Queued mail for delivery/',
        'Amazon_SES' => '/[0-9]{3} Ok (.*)/',
        'SendGrid' => '/[0-9]{3} Ok: queued as (.*)/',
        'CampaignMonitor' => '/[0-9]{3} 2.0.0 OK:([a-zA-Z\d]{48})/',
     ...
     * If null, no HELO string has been received.
     *
     * @var string|null
     */
    protected $helo_rply;
     ...
     * If null, no HELO/EHLO string has been received.
     *
     * @var array|null
     */
    ...
    public function authenticate(
        ...
        if (array_key_exists('EHLO', $this->server_caps)) {
            ...
            if (is_array($this->server_caps['AUTH'])) {
                //If no auth mechanism is specified, attempt to use these, in order
                //Try CRAM-MD5 first as it's more secure than the others
                foreach (['CRAM-MD5', 'LOGIN', 'PLAIN', 'XOAUTH2'] as $method) {
                    if (in_array($method, $this->server_caps['AUTH'], true)) {
                        //Set the chosen auth type
                        if (null === $authtype) {
                            $authtype = $method;
                        }
                        break;
     ...
     * on a single line followed by a CRLF, and the msg body
     * needs to be in the appropriate format specified by RFC2822.
     *
    ...
    public function data($msg_data)
    {
         ...
         * is a header or not, and if so, we will process all header fields first.
         */
    ...
    protected function get_lines()
    {
        ...
        while (is_resource($this->smtp_conn) && !feof($this->smtp_conn)) {
            ...
            //or 4th character is a space or a line break char, then we are done reading, break the loop.
